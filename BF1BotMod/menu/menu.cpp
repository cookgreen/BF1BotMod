...
{
	void tabs(const char* labels[], const int arr_, ImVec2 size, int& cur_page, float x_offset = 0.f, bool* click_callback = false)
	{
		for (int i = 0; i < arr_; i++)
		{
			...
			if (ImGui::Button(labels[i], ImVec2(size.x + x_offset, size.y)))
			{
...
void CMenu::Run()
{
	...
	menu_utils::tabs(labels, IM_ARRAYSIZE(labels), ImVec2(ImGui::GetWindowSize().x / 4.f - 44.f, 20), m_Page);
	...
	else if (m_Page == PAGE_GLOBALS_VISUALS)
	{
		...
		static const char* pcszPlayerType[] = { "Enemies", "Enemies and teammates", "Off", };
		...
		{
			static const char* pcszFontType[] = { "Default", "Shadow", "Outline" };
